...
(task-options!
  pom {:project     'respo/border
       :version     +version+
       :description "Workflow"
       :url         "https://github.com/respo-mvc/respo-border"
       :scm         {:url "https://github.com/respo-mvc/respo-border"}
...
(deftask dev []
  ...
  (comp
    ...
    (watch)
    (transform-cirru)
    (reload :on-jsload 'respo-border.core/on-jsload
            :cljs-asset-path ".")
    (cljs)
    (target)))
...
(deftask rsync []
  (with-pre-wrap fileset
    (sh "rsync" "-r" "target/" "tiye:repo/respo-mvc/respo-border" "--exclude" "main.out" "--delete")
    fileset))
...
(deftask watch-test []
  ...
  (comp
    (watch)
    (transform-cirru)
    (test :namespaces '#{respo-border.test})))
